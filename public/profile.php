...
// Fetch the role and full name from the `users` table
...
$details_stmt = $conn->prepare("SELECT * FROM $role WHERE user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($role === 'student') {
        ...
        // Check for existing record or insert new record
        ...
        if ($result->num_rows > 0) {
            $update_stmt = $conn->prepare("UPDATE student SET reg_no = ?, academic_year = ?, email_id = ?, phone_no = ?, address = ?, index_no = ? WHERE user_id = ?");
            $update_stmt->bind_param("ssssssi", $reg_no, $academic_year, $email_id, $phone_no, $address, $index_no, $user_id);
        } else {
            $update_stmt = $conn->prepare("INSERT INTO student (user_id, reg_no, academic_year, email_id, phone_no, address, index_no) VALUES (?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($result->num_rows > 0) {
            $update_stmt = $conn->prepare("UPDATE mentor SET email_id = ?, phone_no = ?, address = ?, working_organization = ? WHERE user_id = ?");
            $update_stmt->bind_param("ssssi", $email_id, $phone_no, $address, $organization, $user_id);
        } else {
            $update_stmt = $conn->prepare("INSERT INTO mentor (user_id, email_id, phone_no, address, working_organization) VALUES (?, ?, ?, ?, ?)");
        ...
        if ($result->num_rows > 0) {
            $update_stmt = $conn->prepare("UPDATE staff SET email_id = ?, phone_no = ?, address = ? WHERE user_id = ?");
            $update_stmt->bind_param("sssi", $email_id, $phone_no, $address, $user_id);
        } else {
            $update_stmt = $conn->prepare("INSERT INTO staff (user_id, email_id, phone_no, address) VALUES (?, ?, ?, ?)");
