...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['sign_off_report'])) {
    ...
    if ($stmt->execute()) {
        echo "<p class='success-msg'>Report signed successfully!</p>";
    } else {
        echo "<p class='error-msg'>Failed to sign the report. Please try again.</p>";
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_feedback'])) {
    ...
    $month = $_POST['month']; // Ensure you get the month from the form input
    $year = $_POST['year']; // Ensure you get the year from the form input
    $feedback = $_POST['feedback'];
    $mentor_mark = $feedback_marks[$feedback] ?? null; // Get the mark based on feedback

    // Update the diary entry for the specified student, week, month, and year
    $stmt = $conn->prepare(
        "UPDATE diaries 
         SET reviewed = 1, feedback = ?, mentor_mark = ? 
         WHERE student_id = ? AND week_number = ? AND month = ? AND year = ?"
    );
...
$reports = $conn->query(
    "SELECT r.report_id, r.student_id, u.full_name, r.submission_date, r.summary, r.challenges, r.improvements 
     ...
     JOIN student s ON r.student_id = s.user_id 
     WHERE r.status = 'pending' 
       AND s.mentor_id = '$mentor_id'
...
$diaries = $conn->query(
    "SELECT d.* 
     FROM diaries d
     JOIN student s ON d.student_id = s.user_id 
     WHERE d.reviewed = 0 
       AND s.mentor_id = '$mentor_id' 
